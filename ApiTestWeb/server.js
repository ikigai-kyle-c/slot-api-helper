...
function detectMaintenanceBlock(step, status, body) {
  const message = typeof body === "object" && body
    ...
    : "";
  const code = typeof body === "object" && body
...
function cleanupOldResults(flow = "bet") {
  ...
  const files = fs.readdirSync(config.resultDir)
...
function listRecords(flow = "bet") {
  const config = recordConfig(flow);
  if (!fs.existsSync(config.resultDir)) return [];
  return fs.readdirSync(config.resultDir)
    .filter((name) => config.pattern.test(name))
    .sort()
    .reverse()
...
function extractSessionToken(body) {
  return body?.token || body?.sessionToken || body?.data?.token || extractLaunchToken(body?.data?.launchUrl);
}
...
async function refreshRgsTokens(config, logs) {
  ...
  const startHeadersOverride = optionalObjectJson(config, "sessionStartHeadersJson", "Session start headers JSON");
  const startBodyOverride = optionalObjectJson(config, "sessionStartBodyJson", "Session start body JSON");
  ...
  if (start.status !== 200) {
    ...
    throw httpError(`Session start failed with HTTP ${start.status}.`, {
      step: "session.start",
      status: start.status,
      response: start.body,
    }, logs);
  ...
  const activateHeadersOverride = optionalObjectJson(config, "sessionActivateHeadersJson", "Session activate headers JSON");
  const activateBodyOverride = optionalObjectJson(config, "sessionActivateBodyJson", "Session activate body JSON");
  const activatePayload = resolveTemplates(activateBodyOverride || {
    token: sessionToken,
    ts: 0,
    timezone: "us",
    analytics: {
  ...
  if (activate.status !== 200) {
    ...
    throw httpError(`Session activate failed with HTTP ${activate.status}.`, {
...
function getRgsCache(config) {
  ...
  if (cache.baseUrl !== normalizeBaseUrl(config.apiDomain)) return cacheMiss("Cached RGS token baseUrl does not match.");
  if (cache.gameCode !== config.gameCode) return cacheMiss("Cached RGS token gameCode does not match.");
  if (!cache.accessToken || !cache.sessionToken) return cacheMiss("Cached RGS token is missing token fields.");
  if (tokenExpiresSoon(cache.accessToken)) return cacheMiss("Cached RGS access token expired by JWT exp. Refreshing.");
  return { cache, reason: "" };
}
...
async function runLobbyFlow(input) {
  ...
  logs.push({ step: "token.refresh", message: "Starting RGS session and activating game access token." });
  const rgsToken = await refreshRgsTokens(config, logs);

  const tokenActivateHeadersOverride = optionalObjectJson(config, "tokenActivateHeadersJson", "Token activate headers JSON");
  ...
  const tokenActivateBody = tokenActivateBodyRaw === null
    ? null
  ...
  if (activate.status !== 200) {
    ...
    throw httpError(`Session-token activate failed with HTTP ${activate.status}.`, {
  ...
  if (!lobbyAccessToken) throw httpError("Cannot extract lobby access token from data.accessToken.", { step: "session-token.activate", response: activate.body }, logs);
  if (!lobbyRefreshToken) throw httpError("Cannot extract lobby refresh token from data.refreshToken.", { step: "session-token.activate", response: activate.body }, logs);

  const tokenRefreshHeadersOverride = optionalObjectJson(config, "tokenRefreshHeadersJson", "Token refresh headers JSON");
  const tokenRefreshBodyOverride = objectOrNull(parseOptionalJson(config.tokenRefreshBodyJson, "Token refresh body JSON"), "Token refresh body JSON");
  ...
  logs.push({ step: "session-token.refresh.wait", seconds: 5, message: "Waiting 5 seconds before refresh." });
  await delay(5000);
  ...
  if (refresh.status !== 200) {
    ...
    throw httpError(`Session-token refresh failed with HTTP ${refresh.status}.`, {
...
async function runBetFlow(input) {
  ...
  if (!cache) {
    ...
    logs.push({ step: "token.cache", message: "Using cached RGS token.", sessionId: cache.sessionId });
  }
  ...
  if (isInvalidToken(bet.status, bet.body)) {
    logs.push({ step: "token.refresh", message: "Cached RGS token is invalid. Refreshing and retrying bet." });
    cache = await refreshRgsTokens(config, logs);
    bet = await sendBet();
  }
  if (bet.status !== 200) {
    logs.push(blockedLog("play.bet", bet.status, bet.body));
    throw httpError(`Bet failed with HTTP ${bet.status}.`, {
      step: "play.bet",
      status: bet.status,
      response: bet.body,
    }, logs);
  ...
  if (action !== undefined && action !== null && action !== "") {
    const actionHeadersOverride = optionalObjectJson(config, "actionHeadersJson", "Action headers JSON");
    ...
    const actionMaintenanceBlock = detectMaintenanceBlock("play.action", actionResponse.status, actionResponse.body);
    ...
    if (actionResponse.status !== 200) {
      ...
      throw httpError(`Action failed with HTTP ${actionResponse.status}.`, {
        step: "play.action",
  ...
  if (coins !== null && coins > 0) {
    const finishHeadersOverride = optionalObjectJson(config, "finishHeadersJson", "Finish headers JSON");
    ...
    if (!finishPayload.roundId) throw new Error("Play response has summary.coins > 0 but no data.roundId.");
    const finishHeaders = {
      ...playHeaders(cache.accessToken),
    ...
    const finishMaintenanceBlock = detectMaintenanceBlock("play.finish", finish.status, finish.body);
    ...
    if (finish.status !== 200) {
      ...
      throw httpError(`Finish failed with HTTP ${finish.status}.`, {
        step: "play.finish",
    ...
    logs.push({ step: "play.finish.skip", message: `summary.coins=${coins ?? "missing"}, finish not sent.` });
  }
...
function getAmCache(config) {
  ...
  if (cache.baseUrl !== normalizeBaseUrl(config.apiDomain)) return cacheMiss("Cached AM token baseUrl does not match.");
  ...
  if (cache.routeKey !== config.routeKey) return cacheMiss("Cached AM token routeKey does not match.");
  if (String(cache.userId) !== String(config.userId)) return cacheMiss("Cached AM token userId does not match.");
  if (!cache.token || !cache.sessionId) return cacheMiss("Cached AM token is missing token fields.");
  if (tokenExpiresSoon(cache.token)) return cacheMiss("Cached AM token expired by JWT exp. Refreshing.");
  return { cache, reason: "" };
}

async function refreshAmToken(config, logs) {
  const baseUrl = normalizeBaseUrl(config.apiDomain);
  const customHeaders = objectOrNull(parseOptionalJson(config.amTokenHeadersJson, "AM token headers JSON"), "AM token headers JSON");
  const customBody = objectOrNull(parseOptionalJson(config.amTokenBodyJson, "AM token body JSON"), "AM token body JSON");
  const vars = {
    SIGNATURE: config.signature,
  ...
  if (response.status !== 200) {
    throw httpError(`AM token failed with HTTP ${response.status}.`, {
      step: "am.token",
...
async function runMaintenanceFlow(input) {
  ...
  if (!cache) {
    ...
    logs.push({ step: "token.cache", message: "Using cached AM token.", sessionId: cache.sessionId });
  }

  async function sendPatch() {
    const customHeaders = objectOrNull(parseOptionalJson(config.maintenanceHeadersJson, "Maintenance headers JSON"), "Maintenance headers JSON");
    const customBody = objectOrNull(parseOptionalJson(config.maintenanceBodyJson, "Maintenance body JSON"), "Maintenance body JSON");
    const vars = {
  ...
  if (isInvalidToken(patch.status, patch.body)) {
    logs.push({ step: "token.refresh", message: "Cached AM token is invalid. Refreshing and retrying patch." });
    cache = await refreshAmToken(config, logs);
    patch = await sendPatch();
  }
  if (patch.status !== 200) {
    throw httpError(`Maintenance patch failed with HTTP ${patch.status}.`, {
